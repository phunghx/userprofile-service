 ...
 */
object ZConfig {
  val DEV_MODE = "development"
  val PROD_MODE = "production"

  val mode = System.getProperty("mode", DEV_MODE)
  val config = ConfigFactory.load().withFallback(
    ConfigFactory.parseFile(new File("conf/" + mode + ".conf"))
  )
  println(s"Env: $mode")

  def isDevelopmentMode = DEV_MODE.equalsIgnoreCase(mode)
  ...
  def getConfig(s: String, defaultVal: Config): Config = if (hasPath(s)) config.getConfig(s) else defaultVal
